...
param(
    ...
    # Deprecated no-op: scrcpy (live screen mirror) is now installed by default.
    # Kept for backward compatibility; use -Skip Scrcpy to exclude it instead.
    [switch]$SetupScrcpy,
    ...
    [string]$Skip = "",
    # Folder of pre-bundled tool artifacts shipped with the installer. When set and
    # an expected file is present there, it is used instead of downloading (keeps
    # installs offline-capable and avoids the download-and-run pattern AV flags).
    # Expected names: platform-tools.zip, temurin-jre.zip, jadx.zip, apktool.jar,
    # apktool.bat, scrcpy.zip. Missing files fall back to their normal download.
    [string]$BundleDir = ""
)
...
$validSkip = @("Adb", "Java", "Jadx", "Apktool", "Frida", "Scrcpy")
...
# Return the pre-bundled artifact path if -BundleDir has it, else $null (the
# caller then downloads as usual).
function Get-Bundled($name) {
    if ($BundleDir -and (Test-Path (Join-Path $BundleDir $name))) {
...
if ($SkipList -contains "Adb") {
    ...
    $b = Get-Bundled "platform-tools.zip"
    if ($b) { Copy-Item $b $ptZip -Force; Write-Ok "using bundled platform-tools.zip" }
    else {
...
if ($SkipList -contains "Java") {
    ...
    if ($jmajor -ge 11) {
        ...
        $b = Get-Bundled "temurin-jre.zip"
        if ($b) { Copy-Item $b $jreZip -Force; Write-Ok "using bundled temurin-jre.zip" }
        else {
            Invoke-WebRequest -UseBasicParsing `
...
if ($SkipList -contains "Jadx") {
    ...
    $jadxZip = Join-Path $downloadDir "jadx.zip"
    $b = Get-Bundled "jadx.zip"
    if ($b) {
        Copy-Item $b $jadxZip -Force; $ver = "(bundled)"
        Write-Ok "using bundled jadx.zip"
    } else {
    ...
    Write-Ok "JADX $ver at $($jadxBat.FullName) (JADX_PATH set)"
    $summary["JADX"] = "installed $ver"
}
...
if ($SkipList -contains "Apktool") {
    ...
    $bJar = Get-Bundled "apktool.jar"
    $bBat = Get-Bundled "apktool.bat"
    if ($bJar -and $bBat) {
        Copy-Item $bJar (Join-Path $apktoolDir "apktool.jar") -Force
        Copy-Item $bBat (Join-Path $apktoolDir "apktool.bat") -Force
        $ver = "(bundled)"
        Write-Ok "using bundled apktool.jar/apktool.bat"
    } else {
    ...
    Write-Ok "apktool $ver at $apktoolBat (APKTOOL_PATH set)"
    $summary["apktool"] = "installed $ver"
}
...
# ===========================================================================
# 7. scrcpy - live device screen mirror (start_screen_mirror). Default; -Skip Scrcpy to exclude.
# ===========================================================================
# Installed by default (like ADB/Java/JADX); exclude with -Skip Scrcpy. The
# -SetupScrcpy switch is kept for backward compatibility (now a no-op: scrcpy is
# already part of the default setup).
if ($SkipList -contains "Scrcpy") {
    Write-Step "Skipping scrcpy"
    $summary["scrcpy"] = "skipped"
} elseif ($env:SCRCPY_PATH -and (Test-Path $env:SCRCPY_PATH)) {
    ...
    $scrcpyZip = Join-Path $downloadDir "scrcpy.zip"
    $b = Get-Bundled "scrcpy.zip"
    if ($b) {
        Copy-Item $b $scrcpyZip -Force; $ver = "(bundled)"
        Write-Ok "using bundled scrcpy.zip"
    } else {
        $rel = Invoke-RestMethod -UseBasicParsing `
    ...
    Write-Ok "scrcpy $ver at $($scrcpyExe.FullName) (SCRCPY_PATH set)"
    $summary["scrcpy"] = "installed $ver"
}
