...
- `start_screen_mirror(max_size, record)` — open a real-time scrcpy mirror of the active device ON THE ANALYST PC (mouse/keyboard control it); `record=True` saves an mp4 for evidence. Use to watch a sample run live. scrcpy is installed by default by the setup (re-run the installer if missing); if the screen is off, `wake_device` first.
...
- **Never run unbounded/blocking shell commands.** A full-tree `su 0 find /data/data`, `find /` , or a non-`-d` `logcat` produces unbounded output or blocks — the call times out and disrupts the session. Always scope tightly: a specific subpath, `-maxdepth N`, `| head`, `logcat -d -t N`. To inspect an app's data dir, prefer `run-as <pkg> ls files/ cache/` (debuggable) or list a specific subdir; to detect dropped files/sockets/new packages use `capture_baseline`/`diff_baseline` and pull specific files with `pull_file` rather than scanning the whole filesystem. The tool has a `timeout` (default 60s); raise it only for a command that is legitimately slow, don't use it to widen a scan.
