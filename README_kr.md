...
> 전체 도구 정본 목록: [docs/TOOLS.md](docs/TOOLS.md)
...
| `get_server_version` | Android MCP 서버 버전(버전+git 커밋) — 보고서 헤더/버그리포트용 | 없음 |
...
| `wake_device` | 화면 켜기 + (선택) 잠금화면 해제. 깨어있으면 no-op | `unlock` |
...
### 베이스라인 before/after/restore (3개)

기기를 스냅샷·비교하고, 분석 후 **베이스라인으로 회귀(잔존물 삭제)**합니다. 서버 구동 시 `session` 베이스라인을 자동 캡처하고, 분석마다 새 `pre`를 찍습니다(기기 변경 대비). 스냅샷은 workspace 저장(지식카드 아님).
...
| `restore_baseline` | 분석 종료 정리: `after`가 추가한 것을 제거해 `before`로 복원(신규 패키지 uninstall·신규 파일 rm·device-admin 해제·설정 원복) 후 재검증. `apply=False`=드라이런. 기기 불일치 시 거부 | `before`, `after`, `apply` |
...
분석가 PC에 별도 창으로 기기 화면을 **실시간 저지연 미러**(+조작). Claude 대화창 안이 아니라 호스트 창. scrcpy는 설치 시 **기본 포함**(install-inetcop 재실행 또는 `0-setup_environment.ps1`; 제외는 `-Skip Scrcpy`).
...
### 리포트 그림 (3개) — 보고서 증거 이미지

> 저장 경로는 `<workspace>/reports` 기본, `reports.output_dir`(또는 `MCP_REPORTS_DIR`)로 지정 가능(보고서 렌더러가 읽는 공유 경로로 통일용).
...
| `render_flow_diagram` | 클래스/함수 흐름도 PNG(어두운 테마, 악성경로 빨강·보조 슬레이트). Graphviz 불필요. "악성 행위 정보" 최상단용 | `nodes`, `edges`, `malicious`, `aux`, `title` |
