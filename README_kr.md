...
| 도구 | 기능 | 입력 |
|------|------|------|
| `repackage_apk_frida` | frida-gadget 삽입 + 사용자 CA 신뢰 + 재서명(무루트 동적분석용) | `target`, `arch`, `trust_user_certs`, `gadget_config`, `output_path` |

> 준비물(frida-gadget `.so` + uber-apk-signer)은 `1-setup_frida_server.ps1 -SetupFridaServer`가 자동 다운로드합니다. 산출물은 `install_apk`로 설치 → attach.
