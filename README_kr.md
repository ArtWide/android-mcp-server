...
### androguard 정적 분석 (3개) — 루팅 불필요

| 도구 | 기능 | 입력 |
|------|------|------|
| `analyze_manifest` | 권한·**exported 컴포넌트**·debuggable/allowBackup·SDK 레벨 | `package_name` |
| `apk_info` | 서명 인증서·SHA-256·버전·서명 여부 | `package_name` |
| `scan_secrets` | dex 문자열에서 API 키·토큰·URL·IP 등 시크릿 스캔 | `package_name` |
...
### mitmproxy 네트워크 캡처 (4개)

| 도구 | 기능 | 입력 |
|------|------|------|
| `network_start_capture` | mitmdump 기동 + adb reverse + 기기 프록시 설정 | `port` |
| `network_list_flows` | 캡처된 플로우(메서드/상태/URL/크기) 조회 | `limit` |
